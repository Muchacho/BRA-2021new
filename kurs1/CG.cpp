	...
	for (int i = 0; i < idTable.size; i++)
		if (idTable.table[i].idtype == IT::IDTYPE::L)
		{
			...
			if (idTable.table[i].iddatatype == IT::IDDATATYPE::SYM)
				out << " BYTE " << '\'' << idTable.table[i].value.vsym.sym << '\'' << ", 0";
...
void CG::Generation::code()
{
	...
	for (int i = 0; i < lexTable.size; i++) {
		...
		case LEX_FUNCTION: {
			if (lexTable.table[i - 2].lexema == LEX_CREATE)
				break;
			if (func || main)
				break;
			indOfFunc = i + 1;
		...
		case LEX_CICLE_DO: {
			...
			int kc = i, reskc;
			while (kc) { 
				if(lexTable.table[kc].lexema == LEX_CICLE_WHILE){
					reskc = kc;
					break;
				}
				kc++;
			...
			std::string lexzn;
			if (lexTable.table[reskc + 2].lexema == LEX_LITERAL) {
				lexzn = idTable.table[lexTable.table[reskc + 2].idxTI].id;
			}
			else if (lexTable.table[reskc + 2].lexema == LEX_ID) {
				lexzn = "_" + idTable.table[lexTable.table[reskc + 2].idxTI].scope + idTable.table[lexTable.table[reskc + 2].idxTI].id;
			}
			out << "\n\tmov\tecx, " << lexzn;
			out <<  "\n";
			out << "\ncicle" << ciclekol << ":\n\n\tpush ecx\n";
