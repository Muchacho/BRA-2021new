...
#pragma region GRB_rules	
	Greibach greibach(NS('S'), TS('$'),				//стартовый символ, дно стека
		...
		Rule(NS('S'), GRB_ERROR_SERIES + 0,			//Неверная структура программы
			9,		//S →	m{NrE;};	|	tfi(F){NrE;};S	|	m{NrE;};S	|	tfi(F){NrE;}; | i{NrE;} | dtfi(F);S
			...
			Rule::Chain(9, TS('d'), TS('t'), TS('f'), TS('i'), TS('('), NS('F'), TS(')'), TS(';'), NS('S'))
		),
		Rule(NS('N'), GRB_ERROR_SERIES + 1,			//Ошибочный оператор
			20,		//N	→	dti; | rE; | i = E; | dtfi(F); | dti;N | rE;N | i=E;N | dtfi(F);N | pE; | pE;N
			...
			Rule::Chain(9, TS('a'), TS('['), NS('N'), TS(']'), TS('z'), TS('('), NS('W'), TS(')'), TS(';')),
			Rule::Chain(10, TS('a'), TS('['), NS('N'), TS(']'), TS('z'), TS('('), NS('W'), TS(')'), TS(';'), NS('N'))
		),
